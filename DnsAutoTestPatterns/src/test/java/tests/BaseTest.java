...
import webdriverfactory.OptionName;
...
public class BaseTest {
  ...
  public void setUp() {
    ...
    String browser = System
        ...
        .toLowerCase();
    // Получаем параметр запуска тестов через Maven -Doption
    String option = System
        .getProperty("option", "normal")
        .toLowerCase();
    // Получаем экземпляр драйвера браузера
    driver = WebDriverFactory.getDriver(BrowserName.fromString(browser), OptionName.fromString(option));
